...
tf.compat.v1.app.flags.DEFINE_string('img_id', 'girl.jpg', 'File (Image) to process')
...
def main(argv=None):
    ...
    face_files, filenames = detector.run(os.path.join(uploads_path, FLAGS.img_id))
    ...
    if len(face_files) != 0:
        ...
        out_faces = os.path.join(uploads_path, 'faces')
        ...
        for file_name in file_names:
            ...
            if os.path.exists(filepath):
                ...
                shutil.move(os.path.join(output_path, file_name), out_faces)
    else:
        data = {}
        data['people'] = {}
        relationshipPredictor = RelationshipPredictor(data['people'])
